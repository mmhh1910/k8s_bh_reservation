...
BH_USER = os.getenv('BH_USER_SH')
BH_PASSWORD = os.getenv('BH_PASSWORD_SH')
...
try:
    ...
    if path.exists("/data/buecherhalle_sh.pkl"):
        with open("/data/buecherhalle_sh.pkl", "rb") as f:
            entries = pickle.load(f)
            print("/data/buecherhalle_sh.pkl loaded. Entries: "+str(len(entries)))
        ...
        for searchterm in SEARCHTERMS:
            if sr.text.find(searchterm) > -1 and sr.text not in entries:
                ...
                if driver == None:
                    ...
                    # Login
                    driver.get(
                        "https://www.onleihe.de/schleswig_holstein/frontend/login,0-0-0-800-0-0-0-0-0-0-0.html?libraryId=687")
                ...
                driver.get(
                    'https://www.onleihe.de/schleswig_holstein/frontend/'+link.attrs['href'])
                ...
                if ausleihen != None:
                    ...
                    send_mail(
                        "mm@marcusmoennig.de",
                        "Buecherhalle SH Update - Ausgeliehen: " + sr.text,
                        "Buecherhalle SH Update - Ausgeliehen: " + sr.text,
                        "Buecherhalle SH Update - Ausgeliehen: " + sr.text,
                    )
                else:
                    if vormerken != None:
                        ...
                        send_mail(
                            SMTP_TO,
                            "Buecherhalle SH Update - Vorgemerkt: " + sr.text,
                            "Buecherhalle SH Update - Vorgemerkt: " + sr.text,
                            "Buecherhalle SH Update - Vorgemerkt: " + sr.text,
                        )
    ...
    with open("/data/buecherhalle_sh.pkl", "wb") as f:
        pickle.dump(entries, f)
        print("/data/buecherhalle_sh.pkl saved. Entries: "+str(len(entries)))
    ...
    send_mail(SMTP_TO, "Exception searching for buecherhallen sh update ",
